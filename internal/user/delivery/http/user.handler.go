...
func NewHandler(router *gin.RouterGroup, repo user.Repo, logger *zap.Logger) *Handler {
	return &Handler{
		Repo:      repo,
...
func (h *Handler) RetrieveByID() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if id == "" {
			...
			utils.StandardResponse(c, http.StatusBadRequest, "Invalid ID")
			return
		}
		...
		if org == nil {
			utils.StandardResponse(c, http.StatusNotFound, "User not found")
			return
		}
...
func (h *Handler) RetrieveByOutletID() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		users, err := h.Repo.RetrieveByOutletID(c, id, &req)
		if err != nil {
			h.logger.Error("Error retrieving organisation", zap.Error(err))
			utils.StandardResponse(c, http.StatusInternalServerError, err.Error())
			return
		...
		utils.StandardResponse(c, http.StatusOK, users)
	}
}
